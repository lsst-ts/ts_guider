...
void calc_hsm (void *src_buffer, int roinum, int saturation,double *centx, double *centy);

double fmax(double a, double b);
static int imin(int a, int b);
double fX[400], fV[400], fH[400];
double acc[AMAX], alim[AMAX];
int image_bk, image_mx, image_tc;
int nsaturated;
double image_fw;
#define MINBORDER 16
#define GUIDER_MAX_ROI 16
#define GUIDER_OK 0
#define GUIDER_LOS -1
...
CCD_ROI CCD_Frame[GUIDER_MAX_ROI];
...
int32_t*  CCD_new_buffer(char *name, int nx, int ny, int idepth)
{
     int nb; 
     int32_t *bptr;
     int i;
     ...
     while (CCD_Frame[i].pixels != NULL && i < GUIDER_MAX_ROI) {
       i++;
     }
  
     if (i < GUIDER_MAX_ROI) {
       nb = nx * ny * idepth;
       bptr = ((int32_t*) malloc(nb));
...
int locateStars(int32_t *cframe,int roisize, int stepsize, int smoothing, int *xmaxat, int *ymaxat)
{
   int bnum;
   int ifound;
   char bufname[64];
...
int calc_profiles(void *src_buffer, int roinum)
{
  ...
  src_32 = (int32_t *) src_buffer;


  lx = GuiderStamps.roi[roinum].xc - GuiderStamps.roi[roinum].xs/2;
  ux = lx + GuiderStamps.roi[roinum].xs; 
  ly = GuiderStamps.roi[roinum].yc - GuiderStamps.roi[roinum].ys/2;
  uy = ly + GuiderStamps.roi[roinum].ys; 
  ...
  for (line = ly; line <= uy; line++) {
    pV[line] = 0;
    for (pixel = lx; pixel <= ux; pixel++) {
      dn = src_32[line * GuiderStamps.framewidth + pixel];
      pH[pixel]  += dn;
      pV[line] += dn;
      GuiderStamps.roi[roinum].histogram[dn] = GuiderStamps.roi[roinum].histogram[dn] +1;
    }
   }
 ...
 * in each of its sides for symetric purposes. Have in mind that only
...
void calc_quadrant (void *src_buffer, int roinum, int saturation, double *centx, double *centy)
{
  ...
  ux = lx + GuiderStamps.roi[roinum].xs; 
  ly = GuiderStamps.roi[roinum].yc - GuiderStamps.roi[roinum].ys/2;
  uy = lx + GuiderStamps.roi[roinum].ys; 
  GuiderStamps.roi[roinum].nsaturated = 0;
  GuiderStamps.roi[roinum].background = -1.0;
  GuiderStamps.roi[roinum].fmin = -1.0;
  GuiderStamps.roi[roinum].fmax = -1.0;
  GuiderStamps.roi[roinum].mean = -1.0;
  ...
  for (line = ly; line <= uy; line++) {
    for (pixel = lx; pixel <= midx; pixel++) {
      mx1 += (int) src_32[line * GuiderStamps.framewidth + pixel];
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > maxI)
        maxI = (int) src_32[line * GuiderStamps.framewidth + pixel];
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > saturation)
          GuiderStamps.roi[roinum].nsaturated++;
    }
  }
  for (line = ly; line <= uy; line++) {
    for (pixel = midx + 1; pixel <= ux; pixel++) {
      mx2 += (int) src_32[line * GuiderStamps.framewidth + pixel];
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > maxI)
        maxI = (int) src_32[line * GuiderStamps.framewidth + pixel];
    }
  }
  for (line = ly; line <= midy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      my1 += (int) src_32[line * GuiderStamps.framewidth + pixel];
    }
  }
  for (line = midy + 1; line <= uy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      my2 += (int) src_32[line * GuiderStamps.framewidth + pixel];
    }
  }
  sumI = mx1 + mx2;
  if (sumI == 0.0) {
    *centx = (double) GuiderStamps.roi[roinum].xc;
    *centy = (double) GuiderStamps.roi[roinum].yc;
  }
  else {
    *centx = (double) (halfbox * (mx2 - mx1)) / (double) (mx1 + mx2) + (double) GuiderStamps.roi[roinum].xc;
    *centy = (double) (halfbox * (my2 - my1)) / (double) (mx1 + mx2) + (double) GuiderStamps.roi[roinum].yc;
  }
  ...
  for (line = ly; line <= uy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      graylevel = (double)src_32[line * GuiderStamps.framewidth + pixel];
  ...
  GuiderStamps.roi[roinum].background = sky;
  GuiderStamps.roi[roinum].mean = avg;
  GuiderStamps.roi[roinum].fmax = (double)maxI;

  if (avg/sky > GuiderStamps.losthresh) {
      lossOfSignal = 0;
  } else {
      lossOfSignal = 1;
  }
  ...
  *centy = 0.0;
  ...
  for (line = ly; line <= uy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      ...
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > saturation)
          GuiderStamps.roi[roinum].nsaturated++;
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > GuiderStamps.roi[roinum].fmax )
          GuiderStamps.roi[roinum].fmax = (int)graylevel;
    }
  }
  sky = sky/4.0;
  maxH = 0.0;
  ...
  GuiderStamps.roi[roinum].background = sky;
  GuiderStamps.roi[roinum].mean = avg;
  GuiderStamps.roi[roinum].fwhm = image_fw;
...
/*----------------------------------------------------------------------
 * calc_hsm - GalSim HSM centroid algorithm.
 *
 *---------------------------------------------------------------------*/
void calc_hsm(void *src_buffer, int roinum, int saturation, double *centx, double *centy)
{
...
void calc_cmoment(void *src_buffer, int roinum, int saturation, double *centx, double *centy)
{
  ...
  *centy = 0.0;
  ...
  for (line = ly; line <= uy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      graylevel = (double)src_32[line * GuiderStamps.framewidth + pixel] - sky;
      y = line - ly + 1;
      x = pixel - lx + 1;
      if (graylevel < 0.0)
        graylevel = 0.0;
...
void calc_plsphot (void *src_buffer, int roinum, int saturation, double *centx, double *centy)
{
  ...
  for (line = ly; line <= uy; line++) {
    for (pixel = lx; pixel <= ux; pixel++) {
      ix[n] = pixel - box_x;
      iy[n] = line - box_y;
      z[n] = (double) src_32[line * GuiderStamps.framewidth + pixel];
      e[n] = 1.0;
      av += z[n];
      if (z[n] > aux) {
	imax = n;
	aux = z[n];
      ...
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > saturation)
          GuiderStamps.roi[roinum].nsaturated++;
      if (((int) src_32[line * GuiderStamps.framewidth + pixel]) > GuiderStamps.roi[roinum].fmax )
          GuiderStamps.roi[roinum].fmax = (int) src_32[line * GuiderStamps.framewidth + pixel];
    }
  }
  av /= n;
  if (av == 0.0) {
  ...
  a[0] = av;
  a[1] = (double) ix[imax] + (double) GuiderStamps.roi[roinum].xc;
  a[2] = (double) iy[imax] + (double) GuiderStamps.roi[roinum].yc;
  a[3] = z[imax] - av;
  a[4] = (double) 1.;
  if (gdrDebug) {
  ...
  *centy = a[2];
  ...
  image_fw = fmaj;
  image_mx = z[imax];
  GuiderStamps.roi[roinum].background = image_bk;
  GuiderStamps.roi[roinum].fwhm = (double)fmaj;
}
	...
	for (k=1;k<=3;k++) {
	  if ( abs(xcntr-xlast) > 0.001 || abs(ycntr-ylast) > 0.001  ) {
	    ...
	    npts = nx * ny;
            ...
            for (i=y1; i<=y2; i++) {
              for (j=x1;j<=x2;j++) {
                graylevel = (double)src_32[i * GuiderStamps.framewidth + j];
                sum += graylevel;
              }
            }
	    mean = sum / nx;
