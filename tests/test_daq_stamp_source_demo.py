...
import numpy as np
import pytest
from lsst.ts.guider.demo.run_daq_stamp_source_demo import run_daq_stamp_source_demo
...
def test_receive_requested_stamps(daq_source, caplog):
...
def test_timeout_stops_stream(daq_source, caplog):
    _, source, _ = daq_source
    run_daq_stamp_source_demo(["--timeout", "0.001"])
    source.stop_stamp_stream.assert_called_once_with()
    assert "timeout — only received 0 stamps" in caplog.text
...
def test_missing_binding_reports_build_context_and_original_cause(monkeypatch):
    monkeypatch.setitem(sys.modules, "guiderGDS", None)
    with pytest.raises(RuntimeError, match="Build the C\\+\\+ Python bindings") as exc:
        run_daq_stamp_source_demo([])
    assert isinstance(exc.value.__cause__, ImportError)
    assert "CCSDAQ" in str(exc.value)
    assert "PYTHONPATH" in str(exc.value)
    assert "LD_LIBRARY_PATH" in str(exc.value)


def test_log_level_suppresses_stamp_info_but_keeps_timeout(daq_source, caplog):
    _, source, _ = daq_source
    run_daq_stamp_source_demo(["--log-level", "warning", "--timeout", "0.001"])
    source.stop_stamp_stream.assert_called_once_with()
    assert "timeout" in caplog.text
    assert "stamps in Python" not in caplog.text
    assert all(record.levelno >= logging.WARNING for record in caplog.records)


def test_invalid_log_level_does_not_start_stream(daq_source):
    binding, _, _ = daq_source
    with pytest.raises(SystemExit) as exc:
        run_daq_stamp_source_demo(["--log-level", "verbose"])
    assert exc.value.code == 2
    binding.DaqStampSource.assert_not_called()
