...
def build_reference_image(stamps: np.ndarray, n_seed_frames: int) -> np.ndarray:
    ...
    n_seed_frames : `int`
        Maximum number of initial frames to combine. Values less than
        or equal to one return the first stamp.
...
def find_candidate_centroids(
    ...
    config : `GuiderTrackerConfig`
        Detection threshold, footprint size, and edge-margin settings.
