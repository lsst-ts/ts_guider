...
class MultiSensorRunner:
    ...
    def lock_references(
        ...
        """Lock using all supplied seed images; return sensors that locked.

        This is an explicit batch attempt. The caller selects the seed
        prefix; no truncation to ``config.seed_frames`` is performed.
        """
        locked: list[str] = []
...
def run_offline(
    ...
    Each sensor tries its first ``seed_frames`` images, then retries
    with one more image at a time until locking succeeds or its input
    is exhausted. The first successful prefix fixes its reference for
    the whole run. Shorter sequences do not attempt locking. All input
    acquisitions in the common range are measured after reference
    selection, including rejected seeds from unsuccessful prefixes.
    """
    ...
    for sensor_name, sequence in sequences.items():
        for seed_count in range(config.seed_frames, sequence.n_stamps + 1):
            if runner.lock_references({sensor_name: sequence.stamps[:seed_count]}):
                locked.append(sensor_name)
                break
    if not locked:
