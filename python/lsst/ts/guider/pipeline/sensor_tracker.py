...
class SensorTracker:
    ...
    Attributes
    ----------
    reference_center : `tuple` [`float`, `float`] or `None`
        Fixed detected position in the seed reference image, in zero-based
        stamp ``(x, y)`` pixels. `None` until reference locking succeeds.
    ...
    def lock_reference(self, seed_stamps: np.ndarray) -> bool:
        ...
        *measure* candidates rather than trust raw flux:
        ...
        seed_stamps : `numpy.ndarray`
            ...
            Use up to ``config.seed_frames`` initial frames; a shorter
            nonempty cube is also accepted. The caller controls how many
            stamps to accumulate before attempting to lock.
        ...
        including when the new attempt fails.
        """
        self.reference_center = None
        self.stamp_shape = None
    ...
    def measure(self, stamp: np.ndarray) -> CentroidMeasurement:
        ...
        measurement : `CentroidMeasurement`
            Centroid in full-stamp amplifier pixels, shape, and quality
            flags. Call ``measurement.offset_from(reference_center)`` for
            the displacement, checking ``passed_quality`` before use.
            A failed fit or non-finite cutout returns an invalid result.
    ...
    def _passes_quality(self, measurement: CentroidMeasurement) -> bool:
        ...
        if not measurement.converged or not measurement.is_finite:
            return False
        if not np.isfinite(
